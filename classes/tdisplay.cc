...
Boolean TDisplay::searchClosestRes(TScreenResolution *res, int x,
                                   int y, int cant, int &pos)
{
 int i, minDif, indexMin, dif;
 int firstXMatch=-1;
 // Look for an exact match of width
 for (i=0; i<cant && (int)res[i].x<(int)x; i++)
    {
     if ((int)res[i].x==(int)x)
       {
        if (firstXMatch==-1) firstXMatch=i;
        if ((int)res[i].y==(int)y)
          {// Exact match
           pos=i;
           return True;
          }
 ...
 if (firstXMatch!=-1)
   {// Return the closest y that match x
    ...
    while (++i<cant && (int)res[i].x==(int)x)
      {
       dif=abs(res[i].y-y);
       if (dif<minDif)
         {
 ...
 indexMin=0;
 minDif=abs(res[0].y-y)+abs((int)res[0].x-(int)x);
 i=1;
 while (i<cant)
   {
